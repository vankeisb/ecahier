...
class Populate extends BaseResolutionFacet {
    ...
    Resolution getResolution(ActionBeanContext actionBeanContext) {
        ...
        def jean = um.getUserByUsername('jean')
        def nicolas = um.getUserByUsername('nicolas')
        def audrey = um.getUserByUsername('audrey')
        def marie = um.getUserByUsername('marie')

        (1..100).each { i ->
            def b = i % 2 == 0
            s.save(new Entry([
                    createdBy: b ? jean : marie,
                    participants: b ? [audrey] : [nicolas, jean],
